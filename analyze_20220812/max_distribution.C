...
void max_distribution(){
  ...
  TH1F *dis = new TH1F("dis", "Distribution of max ADC",200,6000,7000);
  int maxADC = 0;
  int raw_mid = 0;  
  int peak0 = 0;
  int peak1 = 0;
  ...
  ifstream fin;
  fin.open("/afs/cern.ch/user/s/shuaixia/private/PDS_Data/cathodexarapuca/wave6.dat", ios::in | ios::binary);
...
//        h->Fill(j*dtime,raw[j]);
    ...
    if(maxADC<6580 && maxADC>6550)
	peak0 += 1;
    if(maxADC>6580 && maxADC<6630)
	peak1 += 1;
    if(maxADC>6630 && maxADC<6660)
        peak2 += 1;
    if(maxADC>6660 && maxADC<6720)
	peak3 += 1;
...
//	break;
  ...
  cout<<"========================================="<<endl;
  cout<<"Integral of peak 0 ([6550,6580]): "<<peak0<<endl;
  cout<<"Integral of peak 1 ([6580,6630]): "<<peak1<<endl;
  cout<<"Integral of peak 2 ([6630,6660]): "<<peak2<<endl;
  cout<<"Integral of peak 3 ([6660,6720]): "<<peak3<<endl;
  cout<<"========================================="<<endl;
  ...
  gPad->SetLogy();
  dis->Draw("colz");
  dis->GetXaxis()->SetTitle("ADC");
  dis->GetYaxis()->SetTitle("Counts");
  c2->SaveAs("disADC6.png");
