...
//Subtract result of bkg by another bkg---
//To make sure if it is reasonable to average them (assuming they are almost the same)---
...
void check_bkg(){
  ...
  TH1F *diff = new TH1F("diff", "Signal Integrals",350, -100000,600000);
...
//	break;
  ...
  gPad->SetLogy();
  diff->Draw("colz");
  diff->GetXaxis()->SetTitle("Integral[ADC*ns]");
  diff->GetYaxis()->SetTitle("Counts");
