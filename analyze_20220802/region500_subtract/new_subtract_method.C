...
//To integrate the signal and background---
    ...
    region1 = region1 - (sumBgd1+sumBgd2)/6.0;  


    for(int i=3250; i<3500; ++i){
	region2 +=raw[i];
    }
    region2 = region2 - (sumBgd1+sumBgd2)/6.0;  
...
//to shrink result2-----------------------------------
  for(int i=0; i< binNum/7; ++i){
      counts1 += result1->GetBinContent(i);
      counts2 += result2->GetBinContent(i);
  }

  ratio = 1.0*counts1/counts2;
  cout<<"============================"<<endl;
  ...
  for(int i=0; i< binNum; ++i){
      ...
      binContent3 = static_cast <int> (ratio*binContent2);
      result3->SetBinContent(i, binContent3);
  }

//---Test the result of shrunk histos----
    counts1 = 0;
    counts2 = 0;
    counts3 = 0;
    for(int i=0; i<binNum/7; ++i){
        counts1 += result1->GetBinContent(i);
        counts2 += result2->GetBinContent(i);
	counts3 += result3->GetBinContent(i);
    }
    cout<<"The sum (result1) of bin left to zero is: "<<counts1<<endl;
//    cout<<"The sum (result2) of bin left to zero is: "<<counts2<<endl;
    cout<<"The sum (result3) of bin left to zero is: "<<counts3<<endl;
...
//To get the difference of above two histos
  for(int i=0; i< binNum; ++i){
      ...
      binContent3 = result3->GetBinContent(i);
      diff = binContent1 - binContent3;
//      if(diff > 0){
...
//Draw two figures seperately---------------------------
  ...
  gPad->SetLogy();
  result3->Draw("colz");
  result3->GetXaxis()->SetTitle("[ADC*ns]");
  result3->GetYaxis()->SetTitle("Counts");
...
//Draw three figures at the same plot & draw the subtraction---
  TCanvas *c1 = new TCanvas();
  gPad->SetLogy();
  result1->Draw("colz");
  result1->GetXaxis()->SetTitle("[ADC*ns]");
