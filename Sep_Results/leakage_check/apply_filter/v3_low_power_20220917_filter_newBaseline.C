...
void v3_low_power_20220917_filter_newBaseline(){
...
//======General Variables====================  
  float raw[2500] = {0};//record raw data---
  float filter_data[2500] = {0};//to record filtered data---
  float lambda = 50;

  double level[8] = {0};//used to set the threshold---
  double Count0[8] = {0};
  double Count1[8] = {0};
  double Count2[8] = {0};
  double Count3[8] = {0};
...
//      raw.resize(memorydepth);
    ...
    //to get the baseline for each single waveform---
    TH1F *Baseline_hist = new TH1F("Baseline_hist", "Baseline_hist", nADCs/4, 0, nADCs);
    for(int k=0; k<memorydepth; ++k){
        Baseline_hist->Fill(filter_data[k]);
    }
    baseline_bin_num = Baseline_hist->GetMaximumBin();
    baseline_value = Baseline_hist->GetXaxis()->GetBinCenter(baseline_bin_num);

    //draw an example of waveform---
    if(waveNum == 17020){
        cout<<"======Baseline num: "<<baseline_bin_num<<endl;
	cout<<"======Baseline value is: "<<baseline_value<<endl;
	TCanvas *c6 = new TCanvas();
	Baseline_hist->Draw();
        c6->SaveAs("Example_ADC_hist.png");
//	delete tmp;
    }
    ...
    level[0] = baseline_value + 50;
    for(int i=1; i<8; ++i){
        level[i] = level[0]+25*i;
    }

    //counts the up_crossing numbers---
    for(int i=0; i<8; ++i){
...
//---Reset the values of level[], to show new x axis---
  for(int i=0; i<8; ++i){
      level[i] = 25*(i+2);
  }
//======Drawing====================
