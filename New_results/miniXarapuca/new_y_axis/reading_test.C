...
void reading_test(){
  ...
  ifstream fin;
  fin.open("/mnt/d/PDS_data_temp/minixarapuca-wall-copper/wave3.dat", ios::in | ios::binary);
  ...
  while(!fin.fail()){ 
    ...
    if(first_line){
      ...
      first_line=false;
      h = new TH2D("h","h",memorydepth,0,memorydepth*dtime,nADCs,6000,7000); //if you are brave, don't divide by 5 so you have high definition plot :)
    }
